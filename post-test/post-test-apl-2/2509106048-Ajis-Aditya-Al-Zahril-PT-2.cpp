    ...
    do {
        ...
        if (menuAwal == 3) {
            ...
            if (jumlahUser < 50) {
                ...
                cin >> akunBaru.namaUsername;
                cout << "Masukkan NIM (Password): ";
                cin >> akunBaru.passwordNim;
            ...
            while (cobaLogin > 0 && !statusLoginSukses) {
                ...
                for (int i = 0; i < jumlahUser; i++) {
                    if (dbUser[i].namaUsername == inputNama && dbUser[i].passwordNim == inputPassword) {
                    ...
                    if (menuDashboard == 1) {
                        ...
                        do {
                            ...
                            if (menuDb == 1) {
                                ...
                                for (int gap = jumlahKarakter / 2; gap > 0; gap /= 2) {
                                    for (int i = gap; i < jumlahKarakter; i += 1) {
                                        Karakter temp = dbKarakter[i];
                                        int j;
                                        for (j = i; j >= gap && dbKarakter[j - gap].namaKarakter > temp.namaKarakter; j -= gap) {
                                            dbKarakter[j] = dbKarakter[j - gap];
                                        }
                                        dbKarakter[j] = temp;
                                    }
                                }
                                ...
                                if (jumlahKarakter > 0) {
                                    while (low <= high && keywordPencarian >= dbKarakter[low].namaKarakter && keywordPencarian <= dbKarakter[high].namaKarakter) {
                                        if (low == high) {
                                            if (dbKarakter[low].namaKarakter == keywordPencarian) {
                                                posDitemukan = low;
                                                ditemukan = true;
                                            }
                                            break;
                                        ...
                                        int valKey = keywordPencarian[0];
                                        int valLow = dbKarakter[low].namaKarakter[0];
                                        int valHigh = dbKarakter[high].namaKarakter[0];

                                        int pos;
                                        if (valHigh == valLow) {
                                            pos = low;
                                        } else {
                                            pos = low + (((double)(valKey - valLow) / (valHigh - valLow)) * (high - low));
                                        }

                                        if (pos < low) pos = low;
                                        if (pos > high) pos = high;

                                        if (dbKarakter[pos].namaKarakter == keywordPencarian) {
                                            posDitemukan = pos;
                                            ditemukan = true;
                                            break;
                                        }
